...
void things_todo_at_start(void) {
    ...
    if (LOAD_RC) {
        ...
        parsefile(path, (void (*)(char*)) parseexpr);
        free(path);
    }
    ...
    if (IS_INTERACTIVE) {
        ...
        parsefile(path, (void (*)(char*)) &printf);
        free(path);
        DEBUG = temp;
        printdebug("debugging is on. Turn it off with 'debug off'.");
        printerr("errors will be printed like this");
    }
}
...
void things_todo_at_exit(void) {
    ...
    if (append_history(nb_hist_entries, path) == 0)
        ...
        printerrno("appending %d history entries to %s failed", nb_hist_entries, path);
    free(path);
}
