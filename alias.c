 ...
 **/
bool alias_exists(struct alias *alias) {
	if(head == 0) return false;
	bool keepLooping = true;
	char *newAliasKey = alias->key;
	struct alias *curAlias = head;
	while(keepLooping) {
		// If we've finished looping through all the aliases without a problem, return true.
		if(curAlias == 0) return true;
		
		// Iff the current alias matches one we're trying to define, return false.
		if(strcmp(head->key, newAliasKey) == 0)	return false;
